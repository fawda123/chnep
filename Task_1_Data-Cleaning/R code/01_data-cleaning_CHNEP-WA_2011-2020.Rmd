...
# Contents {#top}
  ...
  * [Monitoring stations and strata](#Strata)
...
The data cleaning script implemented herein verifies the cleaning procedures listed above and applies additional procedures. The major innovations/novelties associated with these procedures include
  ...
  * Implementating functions for automated recognition of fatal qualifier codes and non-detect samples, even when the qualifier code string contains multiple characters.
...
```
The `dat` dataframe contains `r prettyNum(nrow(dat),big.mark=",",scientific=FALSE)` rows of long-format water quality data (provided by the USF Water Atlas team) and `r ncol(dat)` columns. The column names are printed below.
```{r}
colnames(dat)
```
The `grid.strata`, `grid.stations`, `code.defs`, and `thresholds` dataframes are described later in the script.
...
The trend analysis will use 10 years of sample data from 2011 through 2020. Below, we subset the data in `df` by date, store this subset as a new dataframe `df1`, and rename the `Activity_Start_Date` column to `Date`.
```{r}
...
```
Units for pH values are labeled "None", and we re-label these units "pH".
```{r}
levels(df1$Result_Unit) <- c( levels(df1$Result_Unit), 'pH' )
df1$Result_Unit[ which(df1$Analyte=='pH') ] <- as.factor('pH')
```
...
```
We discard these `r length(which(df1$Activity_Type %in% c('Equipment Blank','Field Blank')))` blanks and drop the associated factor levels.
```{r}
...
df1 <- df1[ surface.idx, ]
```
A total of `r prettyNum(nrow(df1.nonsurface),big.mark=",",scientific=FALSE)` non-surface records are discarded. Below, we explore the `df1.nonsurface` dataframe, which contains all records discarded as non-surface samples, to ensure that no records are unnecessarily discarded. Although some "Surface" records were discarded, as indicated below, the associated `Depth` values exceed 1 m.
```{r}
...
```
We discard the duplicate records and store the result in a new dataframe `df2`. Calling `duplicated()` confirms that `df2` contains no duplicates. 
```{r}
df2 <- df1[-dupes.1,]  
...
We start by capitalizing all station names (`StationID`) to remove any inconsistencies in capitalization.
```{r}
df2$StationID <- toupper( df2$StationID )
```
Because the water quality data were obtained from various sources (`DataSource`), it is possible that some `StationID` labels are not geographically unique. That is, two data providers may use the same `StationID` label to refer to different real-world sample locations. For instance, Lee County and Cape Coral each provide data from a Station 160, but these actually refer to two distinct locations.
```{r}
df2$DataSource[ which(df2$StationID=='160') ] %>% unique()
```
To resolve this issue, we will create unique station labels by concatenating information about data providers and stations (`DataSource` and `StationID`). Below, we print the unique `DataSource` labels.
```{r}
df2$DataSource %>% unique() %>% sort()
```
We note that several `DataSource` labels indicate both the source database and the data provider. For instance, `WIN_CAPECRD` and `STORET_CAPECRD` both refer to Cape Coral (CAPECRD) as the data provider, even though the data are split between the two FDEP databases: WIN and STORET. For our purposes (i.e., creating unique station labels), we want data from each provider to be treated the same.
We create a new column, `DataProvider`, containing only the data provider component of each `DataSource` by calling `sub()` to remove `WIN_` and `STORET_` from all `DataSource` strings.
```{r}
df2$DataProvider <- df2$DataSource
df2$DataProvider <- sub( "WIN_", "", df2$DataProvider ) # Remove 'WIN' prefix
df2$DataProvider <- sub( "STORET_", "", df2$DataProvider ) # Remove 'STORET' prefix
```
The result is a shorter list of data providers.
```{r}
df2$DataProvider %>% unique() %>% sort()
```
Next, we create unique station labels in a new column, `Unique.StationID`, by concatenating `DataProvider` and `StationID` values.
```{r}
df2$Unique.StationID <- paste0( df2$DataProvider, ": ", df2$StationID )
df2$Unique.StationID %>% unique() %>% head()
```
Monitoring within 13 open-water estuarine areas was conducted according to a spatially stratified random sampling design: Each "stratum" comprises a set of one-square-mile grid cells, and during each sampling event, samples are collected from randomly selected grid cells within each stratum. The data collected from grid cells within each stratum are considered representative of the stratum and should therefore be analyzed together.

Records in this dataset associated with the stratified sampling design were provided by `LEE_WQ`, `SARASOTAES_WQ`, and `CHNEPMP`. Other data from the Southwest Florida Water Management District (`FL21SWFD`) are also associated with the stratified sampling design, but those data were not included as part of this dataset and are therefore cleaned and processed separately in accompanying R Markdown documents. Associations between `Station ID` values, strata grid cell numbers, and strata names are contained in the `grid.stations` and `grid.strata` dataframes loaded from file at the top of this script.

We create a new `Stratum.Grid` column and assign the grid numbers corresponding to the appropriate `StationID` values by calling `plyr::mapvalues()` and using the `grid.stations` dataframe.
```{r}
grid.idx <- which( (df2$DataProvider %in% c('LEE_WQ','SARASOTAES_WQ','CHNEPMP')) &
                   (df2$StationID %in% grid.stations$Station) )
df2$Stratum.Grid <- NA  # initialize new column
df2$Stratum.Grid[ grid.idx ] <- mapvalues( x = df2$StationID[ grid.idx ], # map stations to grids
                                           from = grid.stations$Station,
                                           to = grid.stations$Grid,
                                           warn_missing = FALSE )
```
As expected, we find that non-`NA` entries in the new `Stratrum.Grid` column are associated with three data providers: `LEE_WQ`, `SARASOTAES_WQ`, and `CHNEPMP`.
```{r}
df2$DataProvider[ which(!is.na(df2$Stratum.Grid)) ] %>% unique()
```
Next, we map the grid cell numbers (`Stratum.Grid`) to the strata names in a new column, `Stratum`, using the `grid.strata` dataframe.
```{r}
df2$Stratum <- mapvalues( x = df2$Stratum.Grid,
                          from = grid.strata$Grid,
                          to = grid.strata$Stratum,
                          warn_missing = FALSE )
```
Below, the for-loop prints the `Unique.StationID` labels associated with each `Stratum`. We also create a new vector `n.stations.in.strata` to count the number of stations associated with each stratum.
```{r}
# Check that station-stratum associations are one-to-one or many-to-one
unique.strata <- df2$Stratum %>% unique() %>% sort()
n.stations.in.strata <- c()
for( i in 1:length(unique.strata) ){
  this.stations <- unique( df2$Unique.StationID[which(df2$Stratum==unique.strata[i])] )
  if(length(this.stations)>1){
    cat( paste0( unique.strata[i],'\n   ',paste(this.stations,collapse='\n   ',sep=' '),'\n') )
  }  # // end if()
  n.stations.in.strata <- c( n.stations.in.strata, length(this.stations) )
}
names(n.stations.in.strata) <- unique.strata
n.stations.in.strata
```
Finally, we loop over each `Unique.StationID` label to count the number of strata associated with each station (`n.strata.in.stations`). The output confirms that there is only one stratum per station.
```{r}
# Check that there is no more than one stratum per station
unique.stations <- df2$Unique.StationID %>% unique() %>% sort()
n.strata.in.stations <- c()
for( i in 1:length(unique.stations) ){
  this.stratum <- unique( df2$Stratum[which(df2$Unique.StationID==unique.stations[i])] )
...
```
Finally, we overwrite `Unique.StationID` labels associated with strata with the corresponding `Stratum` label. This change will allow us to use the `Unique.StationID` column to identify distinct locations (stations or strata) for trend analysis.
```{r}
df2$Unique.StationID[ grid.idx ] <- df2$Stratum[ grid.idx ]
```
We verify that the `Unique.StationID` labels were properly overwritten with a couple of logical tests and by printing 20 random rows from `df2`:
```{r}
any( df2$Unique.StationID[-grid.idx] %in% unique.strata ) # confirm no non-strata stations were labeled as strata (FALSE)
all( df2$Unique.StationID[grid.idx] %in% unique.strata ) # confirm all strata grids were labeled as strata (TRUE)
set.seed(8675309)
df2[ grid.idx[sample(1:length(grid.idx),20)], c('Date','Unique.StationID','Stratum') ]
```
The `df2` dataframe contains data from `r length(na.omit(unique(df2$Stratum)))` strata and from `r length(unique(df2$Unique.StationID[ which(is.na(df2$Stratum)) ]))` unique stations that are not associated with strata. Data from other strata (provided by SFWMD via the WIN and STORET databases) are cleaned and processed in accompanying R Markdown documents.
...
We evaluate non-detects reporting `MDL`s and `Result_Value`s in different units. We begin by printing `MDL_Unit`s associated with non-detect samples of each analyte. We find that MDLs for the nitrogen and phosphorus analytes (`TN`, `NH4`, `TKN`, `NOx`, `TP`, and `PO4`) are reported in mg/l, while `Result_Value`s are reported in ug/l.
```{r}
...
```
We print all unique nitrogen and phosphorus `MDL` values to confirm that it is reasonable to trust the "mg/l" unit label. The MDL values are all very low (<1), confirming that they are expressed as mg/l. We therefore convert the MDLs from mg/l to ug/l for consistency with the `Result_Unit`s.
```{r}
...
We evaluate non-detects with no MDL information (no values or units). For each analyte, we print the "target" `Result_Value`s and value range (values with no associated MDL information), the MDL values associated with non-detects, and all MDL values. We find some unexpected `Result_Value`s for `Color`, `NH4`, and `TKN`.
```{r}  
for( i in 1:length(analytes)){
  this.analyte <- analytes[i]
  targets.idx <- which( df3$Analyte==this.analyte &
...
```
We explore the large (>8) `Result_Value`s among non-detect `Color` samples.
```{r}    
...
df3 <- df3[ -which( df3$Analyte=='Color' &
                    df3$Result_Value>8 & df3$Non_detect==TRUE), ]
```
Next, we explore the low `Result_Value`s (0.008) among non-detect `NH4` samples. We find `r length(which(df3$Analyte=='NH4' & df3$Result_Value==0.008 & df3$Non_detect==TRUE))` such records, and they are all labeled with "ug/l" units.
```{r}    
...
```
And finally, we explore the large `Result_Value`s (>100) among non-detect `TKN` samples.
```{r}
...
We identify all non-detects with MDL information and check whether the `Result_Value`s match the corresponding `MDL` values.
```{r}    
...
```
For these `r length(mismatch.idx)` records, we assign the `MDL` value as the `Result_Value`.
```{r}
...
```{r}
    ...
    write.csv(threshold.screening,"threshold.results.CHNEP.csv",row.names=FALSE)
```
...
We export the clean dataset (`df3`), which contains `r prettyNum(nrow(df3),big.mark=",",scientific=FALSE)` rows. We save the data to a csv file and an RData file.
```{r}
write.csv( df3, "clean-data_CHNEP-WA-WQ_2011-2020.csv", row.names=FALSE )
save( df3, file="clean-data_CHNEP-WA-WQ_2011-2020.RData" )
```
