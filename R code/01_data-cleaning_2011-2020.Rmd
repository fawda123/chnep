...
```
Because the water quality data were provided from various agencies (`DataSource`), it is possible that the `StationID` labels are not geographically unique. That is, two data providers may use the same `StationID` to refer to different real-world sample locations. To resolve this issue, we create unique station labels by concatenating the data source (`DataSource`) and station (`StationID`) information and store the unique labels in a new column, `DS.StationID`. And to illustrate, we print the first six unique labels.
```{r}
df2$DS.StationID <- paste( df2$DataSource, ":", df2$StationID )
df2$DS.StationID %>% unique() %>% head()
```
Some monitoring stations are treated as a group, or stratum, and the station-stratum associations are provided in the `strata` dataframe loaded from file at the top of this script. To associate station labels (`df2$DS.StationID`) with the corresponding strata, we call the `plyr::mapvalues()` function, and we store the results in a new column, `df2$Stratum`. Many individual monitoring stations are not grouped into a stratum, and in these cases, the `df2$DS.StationID` labels are retained in the new `Stratum` column. Subsequent checks confirm that each station is associated with only one strata (each stratum can be associated with one or more stations).
```{r}
# Create new column grouping stations by stratum (not all 'strata' stations appear in 'df2')
df2$Stratum <- mapvalues( x=df2$DS.StationID, from=strata$UNIQUEID, to=strata$Stratum, warn_missing=FALSE )
```
Below, the for-loop prints the unique `DS.StationID` values associated with each `Stratum` in `df2`. We also create a new vector `n.stations.in.strata` to count the number of stations associated with each stratum.
```{r}
...
for( i in 1:length(unique.strata) ){
  this.stations <- unique( df2$DS.StationID[which(df2$Stratum==unique.strata[i])] )
  if(length(this.stations)>1){
    cat( paste0( 'Stratum ', unique.strata[i],'\n   ',paste(this.stations,collapse='\n   ',sep=' '),'\n') )
  }  # // end if()
  n.stations.in.strata <- c( n.stations.in.strata, length(this.stations) )
}
```
Finally, we loop over each unique `DS.StationID` value to count the number of strata associated with each station (`n.strata.in.stations`). The output confirms that there is only one stratum per station.
```{r}
# Check that there is no more than one stratum per station
unique.stations <- df2$DS.StationID %>% unique() %>% sort()
n.strata.in.stations <- c()
for( i in 1:length(unique.stations) ){
  this.stratum <- unique( df2$Stratum[which(df2$DS.StationID==unique.stations[i])] )
...
```
There are `r length(unique(df2$DS.StationID))` unique `DS.StationID` labels and `r length(unique(df2$Stratum))` unique `Stratum` labels in `df2`.
...
We export the clean dataset (`df3`), which contains `r nrow(df3)` rows. We save the data to a csv file and an RData file.
```{r}
write.csv( df3, "CHNEP-WQ_clean-data_2011-2020.csv", row.names=FALSE )
save( df3, file="CHNEP-WQ_clean-data_2011-2020.RData" )
```
