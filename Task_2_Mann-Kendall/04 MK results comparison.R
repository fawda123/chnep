...
# Format USF's trend analysis results
  ...
  # Format unique station names consistent with the CCS results
  USF.results$DataSource <- sub( "WIN_", "", USF.results$DataSource ) # remove 'WIN_' prefixes
  USF.results$StationID <- sub( "GSV006         ", "GSV006", USF.results$StationID ) # remove extra spaces from GSV006
  USF.results$Unique.StationID <- paste0( USF.results$DataSource, ": ", USF.results$StationID )  # generate unique station IDs
  # Print Unique.StationID's appearing in USF.results but not appearing in CCS results
  missing.USF <- unique(USF.results$Unique.StationID)[ which( !( unique(USF.results$Unique.StationID) %in%
                                                                 unique(results.df$Station) ) ) ] %>% sort()
  missing.USF
  # Confirm that missing.USF stations are all associated with strata (and therefore should be missing from CCS results)
  match.stations <- logical()
  for( i in 1:length(missing.USF) ){
    this.source <- unique( USF.results$DataSource[ which( USF.results$Unique.StationID==missing.USF[i] ) ] )
    this.station <- unique( USF.results$StationID[ which( USF.results$Unique.StationID==missing.USF[i] ) ] )
    agencies <- data.frame( Name1 = c('SWFWMD','Lee County','Sarasota County','CHNEP'),
                            Name2 = c('SWFWMD','LEE_WQ','SARASOTAES_WQ','CHNEPMP') )
    match.idx <- which( grid.stations$Agency==agencies$Name1[which(agencies$Name2==this.source)] &
  ...
  # Subset columns
  USF.results <- USF.results[ , which( colnames(USF.results) %in% c('Unique.StationID','Analyte','Trend') ) ]
  colnames( USF.results )[ which( colnames(USF.results)=='Unique.StationID' ) ] <- 'Station'
...
# Compare USF and CCS trend results
    ...
    # In a new column, label comparisons of results
    ...
    # Stations labeled 'CCSnotrun.USFrun'
    all.results$Station[ which( all.results$Compare=="CCSnotrun.USFrun" ) ] %>% unique %>% sort
    # Stations labeled 'CCSrun.USFnotrun'
    all.results$Station[ which( all.results$Compare=="CCSrun.USFnotrun" ) ] %>% unique %>% sort
...
# Investigate disagreements in trend direction
    ...
    all.results[ which( !is.na(all.results$USF.MK.trend) &
      ...
      # Unique stations run by USF but not run by CCS
      notrun.stations <- all.results$Station[ which( !is.na(all.results$USF.MK.trend) &
        ...
        for( i in 1:length(notrun.stations) ){
          ...
          this.station <- strsplit( notrun.stations[i], ": " )[[1]][2]
          agencies <- data.frame( Name1 = c('SWFWMD','Lee County','Sarasota County','CHNEP'),
                                  Name2 = c('SWFWMD','LEE_WQ','SARASOTAES_WQ','CHNEPMP') )
          match.idx <- which( grid.stations$Agency==agencies$Name1[which(agencies$Name2==this.source)] &
                                grid.stations$Station==this.station )
